 ...
 * a fresh Analytics call, and only that path consults the rate limiter, since only
 * that path can actually spend anything.
 */
export async function GET(request: Request) {
  try {
    ...
    if (!live) {
      ...
      // No snapshot yet, so fall through to a live call rather than returning
      // nothing on a freshly linked channel.
    }
    ...
    return handleRouteError(err, "channel/analytics");
  }
}
